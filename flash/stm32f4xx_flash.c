...
static uint8_t logic_flashGetSector(uint32_t Address)
{
        uint32_t sector = 0;
...
extern void chSysHalt(const char * reason);
...
flash_error_t flash_erase(uint32_t start, uint32_t size)
{
        ...
        uint32_t EndSector = logic_flashGetSector(start + size);
...
flash_error_t flash_write(uint32_t Address, void * datas, uint32_t size)
{
